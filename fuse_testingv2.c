...
void init_leveldb() {
    ...
    if (err != NULL) {
        dprintf(STDERR_FILENO, "Erro ao abrir o LevelDB: %s\n", err); // Usa dprintf para erro
        leveldb_free(err);
        exit(1);
    }
}

// Função para fechar o LevelDB
void close_leveldb() {
    leveldb_close(db);
...
// Função para listar o conteúdo da diretoria
...
static int my_fuse_create(const char *path, mode_t mode, struct fuse_file_info *fi) {
    ...
    // Insere uma entrada vazia no LevelDB
    char *err = NULL;
    leveldb_put(db, leveldb_writeoptions_create(), path, strlen(path), "", 0, &err);
    if (err != NULL) {
        dprintf(STDERR_FILENO, "Erro ao criar entrada no LevelDB: %s\n", err);
        leveldb_free(err);
        return -EIO;
    }
...
// Função para escrita de ficheiros
static int my_fuse_write(const char *path, const char *buf, size_t len, off_t offset, struct fuse_file_info *fi) {
    ...
    // Atualiza o conteúdo no LevelDB
    char *err = NULL;
    leveldb_writeoptions_t *write_options = leveldb_writeoptions_create();
    leveldb_put(db, write_options, path, strlen(path), buf, len, &err);
    leveldb_writeoptions_destroy(write_options);

    // Verifica se ocorreu algum erro ao escrever no LevelDB
    if (err != NULL) {
        printf("Erro ao escrever no LevelDB: %s\n", err);
        leveldb_free(err);
        return -EIO;
    }
...
int main(int argc, char *argv[]) {
    init_leveldb();
    int res = fuse_main(argc, argv, &my_fuse_operations, NULL);
    close_leveldb();
    return res;
}